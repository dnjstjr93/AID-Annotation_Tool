var cocoFormater = {
    ...
    toCOCO : function(labellingData){
        ...
        var cocoData = {
            images : [],
            "type": "instances", // aidlab: this may be changed ?
            annotations : [],
            categories : []
        }
        ...
        for(var image_i = 0 ; image_i < images.length; image_i++){
            ...
            cocoData.images.push({
                ...
                //"id": image_i+1 
                "id": parseInt(imageName.split('-')[3]), // aidlab by wonseok ********************  // opendataset MATLAB data form >>>>> "id": parseInt(imageName.split('.')[0])
            });
            ...
            for(var shape_i=0; shape_i < labellingData[ imageName ].shapes.length;  shape_i++ ){
                ...
                cocoData.annotations.push({
                    ...
                    num_keypoints: shape.featurePoints.length, // aidlab: we modified the imglab code
                    area : area,
                    "iscrowd": 0,
                    keypoints : makeKeypoints(shape.featurePoints), // aidlab: we modified the imglab code
                    // "image_id": image_i+1,
                    "image_id": parseInt(imageName.split('-')[3]), // aidlab by wonseok ********************  // opendataset MATLAB data form >>>>> "id": parseInt(imageName.split('.')[0])
                    "bbox": [shape.bbox.x, shape.bbox.y, shape.bbox.width, shape.bbox.height],
                    // "category_id": categories.indexOf(shape.category) + 1, 
                    "category_id": 99, // aidlab by wonseok                    
                    "id": shape_i+1,
                    // "ignore": 0 // aidlab by wonseok 
                });
        ...
        for(var category_i = 0 ; category_i < categories.length; category_i++){
            cocoData.categories.push({
                ...
                // "id": category_i+1,
                "id": 99, // aidlab by wonseok
                "name": categories [ category_i ] || "pig",
                "keypoints": ["left_ear","right_ear","shoulder","tail"], // aidlab: we define left_ear 0, right_ear 1, shoulder 2, tail 3
                "skeleton": [[0,2],[1,2],[2,3]]
            })
        }
...
// aidlab: we define a new function
function makeKeypoints(fPoints){
    //var fPoints = fPoints_in;
    var obj_knum = 4; // aidlab: number of keypoints in an object (ex. a pig has four keypoints)
    var coco_knum = 3; // aidlab: in the COCO datasets, each keypoint has three values (x, y, v) where v (visibility) - v=0: not labeled (in which case x=y=0), v=1: labeled but not visible, and v=2: labeled and visible
    var kpoints = new Array(obj_knum*coco_knum).fill(0);

    for(var fPoint_i in fPoints){
         var fPoint = fPoints[fPoint_i]
         kpoints[coco_knum*fPoint.label] = Math.round(fPoint.x)
         kpoints[coco_knum*fPoint.label+1] = Math.round(fPoint.y)
         kpoints[coco_knum*fPoint.label+2] = 2;
    }
